...
namespace Dignite.Wechat.Mp.WebApp
{
    ...
    public class AuthorizationUrlMiddleware
    {
        ...
        public async Task Invoke(HttpContext context)
        {
            ...
            if (!WebAppConsts.AuthorizationPath.Equals(request.Path, StringComparison.OrdinalIgnoreCase))
            {
                await this._next(context);
                return;
            }

            var scope = (AuthencationScope)Enum.Parse(typeof(AuthencationScope), request.Query["scope"].ToString());
